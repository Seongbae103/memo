...
- 확률 해석의 관점에 따라
  - 빈도주의는 
    > - 확률을 빈도로 해석
    > - 데이터가 부족하거나 불확실하다면 결과도 불확실
  - 베이지안
  > - 확률을 믿음의 정도(어느정도 확신할 수 있는가)로 해석
  > - 모수 = 확률적으로 변하는 수(확률변수) → 모델을 만들 때는 주로 베이지안을 사용
  > - 순서 : 사전확률(지식) → 관측결과(데이터) → 가능도(likelihood) → 사후확률<br>
